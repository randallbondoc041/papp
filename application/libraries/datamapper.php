...
class DataMapper implements IteratorAggregate {
	 ...
	 * @return	DataMapper Returns self for method chaining.
	 */
	public function like_open($field, $match = '', $side = 'both')
	{
		return $this->_like($field, $match, 'AND ', $side, '', FALSE, TRUE);
	}
	...
	{
		return $this->_like($field, $match, 'OR ', $side, '', FALSE, FALSE, TRUE);
	}
	 ...
	 * @return	DataMapper Returns self for method chaining.
	 */
	protected function _like($field, $match = '', $type = 'AND ', $side = 'both', $not = '', $no_case = FALSE, $open = FALSE, $closed = FALSE)
	{
		if ( ! is_array($field))
		{
		...
		{
			$close_p = ")";
			$this->db->ar_where[(count($this->db->ar_where))-1] = end($this->db->ar_where) . $close_p;
		}
		// kprint($this->db->ar_where);
		// For method chaining
		return $this;
	}
	 ...
	 * @param   string $join_type The join type either INNER|LEFT|RIGHT...
	 * @return	string Name of the related table, or table.field if ID_Only
	 */
	public function _add_related_table($object, $related_field = '', $id_only = FALSE, $db = NULL, &$query_related = NULL, $name_prepend = '', $this_table = NULL, $join_type = 'LEFT OUTER')
	{
		if ( is_string($object))
		{
		...
		if(strpos($related_field, '/') !== FALSE)
		{
			...
			foreach($rfs as $index => $rf)
			{
				...
				$object_as = $last->_add_related_table($rf, '', $temp_id_only, $this->db, $this->_query_related, $prepend, $object_as, $join_type);
				$prepend .= $rf . '_';
				$last =& $last->_get_without_auto_populating($rf);
			}
			return $object_as;
		...
		if ($relationship_table == $this->table && in_array($other_column, $this->fields))
		{
			...
			{
				$db->join($object->table . ' ' .$object_as, $object_as . '.id = ' . $this_table . '.' . $other_column, $join_type);
		...
		else if ($relationship_table == $object->table && in_array($this_column, $object->fields))
		{
			...
			{
				$db->join($object->table . ' ' .$object_as, $this_table . '.id = ' . $object_as . '.' . $this_column, $join_type);
				$query_related[] = $object_as;
			}
			if($id_only)
			{
		...
		else
		{
			...
			{
				$db->join($relationship_table . ' ' . $relationship_as, $this_table . '.id = ' . $relationship_as . '.' . $this_column, $join_type);
			...
			{
				// Add join if not already included
				$db->join($object->table . ' ' . $object_as, $object_as . '.id = ' . $relationship_as . '.' . $other_column, $join_type);
	 ...
	 * @return	DataMapper Returns self for method chaining.
	 */
	public function include_related($related_field, $fields = NULL, $append_name = TRUE, $instantiate = FALSE, $join_type = 'LEFT OUTER')
	{
		if (is_object($related_field))
		{
		...
		$table = $this->_add_related_table($object, $related_field, FALSE, NULL, $query_related, '', NULL, $join_type);
		$append = '';
